...
* Logs and other transformations can help to **stabilize the variance**.
...
$$
  y_{t} - y_{t-1} = y_t - By_{t} = (1 - B)y_{t}
$$
* A second-order difference is denoted $(1- B)^{2}$.
* In general, a $d$th-order difference can be written as $(1 - B)^{d} y_{t}$
...
```{r pelt1}
pelt %>%
  model(lynx = ARIMA(sqrt(Lynx) ~ pdq(q=0))) %>%
  report()
```
