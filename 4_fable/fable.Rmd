...
```{r beer-stretch, cache=TRUE}
beer_stretch <- aus_production %>%
  stretch_tsibble(.init=1, .step=1)
```

\fontsize{10}{11}\sf
```{r beer_stretch_print, echo = FALSE}
print(beer_stretch)
```
...
```{r beer_fit, cache = TRUE}
fit_cv <- beer_stretch %>% 
  model(arima=ARIMA(Beer), ets=ETS(Beer))
```

\fontsize{10}{11}\sf
```{r beer_fit-print, echo = FALSE, dependson='beer_fit'}
print(fit_cv, n = 30)
```
...
```{r beer_fc, cache = TRUE, dependson='beer_fit'}
fc_cv <- fit_cv %>% filter(.id>8) %>% forecast(h=1)
```

\fontsize{10}{11}\sf
```{r beer_fc-print, echo = FALSE, dependson='beer_fc'}
fc_cv 
```
...
```{r beer_accuracy, cache = TRUE, dependson='beer_fc'}
fc_cv %>% accuracy(aus_production)
```
