...
```{r fceval1a}
tourism_stretch <- tourism_region %>%
...
```{r setwidth2, include=FALSE}
fred <- options(width=85)
```

```{r fceval2, dependson='setwidth2'}
fits <- tourism_stretch %>%
  ...
  reconcile(
    ets_adj = min_trace(ets, method='wls'),
    arima_adj = min_trace(arima, method='wls')
  )
fits
```

```{r setwidthback2, include=FALSE, dependson='setwidth2'}
options(width=fred$width)
```
...
```{r fceval4, message=FALSE}
fc %>% accuracy(tourism_region)
```

## Forecast evaluation
\fontsize{10}{11}\sf

```{r fceval5, message=FALSE}
fc %>% accuracy(tourism_region) %>%
  group_by(.model) %>%
  summarise(MASE = mean(MASE))
```
